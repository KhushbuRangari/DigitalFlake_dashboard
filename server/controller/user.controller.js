...
const resetPassword = async (req, res) => {
  ...
  const forgotPassword = async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ user: user._id }, process.env.SCRETE_KEY, {
        expiresIn: "1h",
      });
  
      // In a real application, send the token via email and handle the reset password flow
      // Example: sendResetTokenByEmail(user.email, token);
